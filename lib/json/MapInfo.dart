import 'package:flutter_template_app/generated/json/base/json_field.dart';
import 'package:flutter_template_app/generated/json/MapInfo.g.dart';

import 'dart:ui';


@JsonSerializable()
class MapInfo {

	MapInfo();

	factory MapInfo.fromJson(Map<String, dynamic> json) => $MapInfoFromJson(json);

	Map<String, dynamic> toJson() => $MapInfoToJson(this);

  /**
   * data : {"projects":[{"map_data":"","map_info":{"map_info":{"height":457,"origin_theta":0,"origin_x":-5.50055380391934,"origin_y":11.983126061027363,"resolution":0.05,"width":582},"pgm_name":"new_map.pgm","png_name":"new_map.png"},"obstacles":{"obstacles":{"circles":[],"polygons":[],"polylines":[],"rectangles":[]}},"positions":{"positions":[{"name":"1","position":{"theta":-1.8065404422243354,"world_x":19.569447416783788,"world_y":-4.297116858894512},"type":2},{"name":"2","position":{"theta":-1.7995632842135576,"world_x":15.331431669713472,"world_y":-3.715154273445293},"type":2},{"name":"3","position":{"theta":-1.7033021006698363,"world_x":11.198516935826754,"world_y":-2.9019874643632626},"type":2},{"name":"4","position":{"theta":-1.7088198985162588,"world_x":4.606330198766207,"world_y":-1.6907569956132633},"type":2},{"name":"5","position":{"theta":-1.8260997249310575,"world_x":0.5096039719595664,"world_y":-0.817002112800763},"type":2}]},"project_info":{"map_name":"map.json","obstacle_name":"obstacle.json","positions_name":"positions.json","project_id":"cfb81e49-bafa-42f0-ab86-69beb515cdbd","project_name":"0711","project_stamp":"2020-07-11T01:50:22.287000Z"},"scripts":{"scripts":[{"data":"[{\"break_point_flag\":{\"type\":\"bool\",\"value\":false},\"id\":{\"type\":\"std::string\",\"value\":\"EGWXFX1HJl\"},\"sub_tasks\":[{\"break_point_flag\":{\"type\":\"bool\",\"value\":false},\"id\":{\"type\":\"std::string\",\"value\":\"M9b62f9Y2P\"},\"input_waypoint\":{\"type\":\"waypoint\",\"value\":{\"name\":{\"type\":\"std::string\",\"value\":\"4\"},\"theta\":{\"type\":\"double\",\"value\":1.5707963267948966},\"x\":{\"type\":\"double\",\"value\":4.606330198766207},\"y\":{\"value\":1.8762420583418162}}},\"required_navi_type\":{\"type\":\"enum\",\"value\":2},\"task_name\":{\"type\":\"std::string\",\"value\":\"Navi\"}},{\"break_point_flag\":{\"type\":\"bool\",\"value\":false},\"id\":{\"type\":\"std::string\",\"value\":\"kT4uHe03IG\"},\"input_waypoint\":{\"type\":\"waypoint\",\"value\":{\"name\":{\"type\":\"std::string\",\"value\":\"5\"},\"theta\":{\"type\":\"double\",\"value\":1.5707963267948966},\"x\":{\"type\":\"double\",\"value\":0.5096039719595664},\"y\":{\"value\":5.972968285148457}}},\"required_navi_type\":{\"type\":\"enum\",\"value\":2},\"task_name\":{\"type\":\"std::string\",\"value\":\"Navi\"}}],\"task_name\":{\"type\":\"std::string\",\"value\":\"Loop\"},\"times\":{\"type\":\"unsigned int\",\"value\":1}}]","script_name":"bzrobot_patrol.script"}]}}]}
   * module : /api/sync/PullSyncProject
   * result_code : 0
   * result_msg : success
   * result_serial_number : 27002D000F50485259353820
   * result_stamp : 2020-07-25T08:22:10.664128Z
   */

  DataBean? data;
  String? module;
  int? result_code;
  String? result_msg;
  String? result_serial_number;
  String? result_stamp;
}

@JsonSerializable()
class DataBean {

	DataBean();

	factory DataBean.fromJson(Map<String, dynamic> json) => $DataBeanFromJson(json);

	Map<String, dynamic> toJson() => $DataBeanToJson(this);

  List<ProjectsBean>? projects;
}

@JsonSerializable()
class ProjectsBean {

	ProjectsBean();

	factory ProjectsBean.fromJson(Map<String, dynamic> json) => $ProjectsBeanFromJson(json);

	Map<String, dynamic> toJson() => $ProjectsBeanToJson(this);

  /**
   * map_data : 
   * map_info : {"map_info":{"height":457,"origin_theta":0,"origin_x":-5.50055380391934,"origin_y":11.983126061027363,"resolution":0.05,"width":582},"pgm_name":"new_map.pgm","png_name":"new_map.png"}
   * obstacles : {"obstacles":{"circles":[],"polygons":[],"polylines":[],"rectangles":[]}}
   * positions : {"positions":[{"name":"1","position":{"theta":-1.8065404422243354,"world_x":19.569447416783788,"world_y":-4.297116858894512},"type":2},{"name":"2","position":{"theta":-1.7995632842135576,"world_x":15.331431669713472,"world_y":-3.715154273445293},"type":2},{"name":"3","position":{"theta":-1.7033021006698363,"world_x":11.198516935826754,"world_y":-2.9019874643632626},"type":2},{"name":"4","position":{"theta":-1.7088198985162588,"world_x":4.606330198766207,"world_y":-1.6907569956132633},"type":2},{"name":"5","position":{"theta":-1.8260997249310575,"world_x":0.5096039719595664,"world_y":-0.817002112800763},"type":2}]}
   * project_info : {"map_name":"map.json","obstacle_name":"obstacle.json","positions_name":"positions.json","project_id":"cfb81e49-bafa-42f0-ab86-69beb515cdbd","project_name":"0711","project_stamp":"2020-07-11T01:50:22.287000Z"}
   * scripts : {"scripts":[{"data":"[{\"break_point_flag\":{\"type\":\"bool\",\"value\":false},\"id\":{\"type\":\"std::string\",\"value\":\"EGWXFX1HJl\"},\"sub_tasks\":[{\"break_point_flag\":{\"type\":\"bool\",\"value\":false},\"id\":{\"type\":\"std::string\",\"value\":\"M9b62f9Y2P\"},\"input_waypoint\":{\"type\":\"waypoint\",\"value\":{\"name\":{\"type\":\"std::string\",\"value\":\"4\"},\"theta\":{\"type\":\"double\",\"value\":1.5707963267948966},\"x\":{\"type\":\"double\",\"value\":4.606330198766207},\"y\":{\"value\":1.8762420583418162}}},\"required_navi_type\":{\"type\":\"enum\",\"value\":2},\"task_name\":{\"type\":\"std::string\",\"value\":\"Navi\"}},{\"break_point_flag\":{\"type\":\"bool\",\"value\":false},\"id\":{\"type\":\"std::string\",\"value\":\"kT4uHe03IG\"},\"input_waypoint\":{\"type\":\"waypoint\",\"value\":{\"name\":{\"type\":\"std::string\",\"value\":\"5\"},\"theta\":{\"type\":\"double\",\"value\":1.5707963267948966},\"x\":{\"type\":\"double\",\"value\":0.5096039719595664},\"y\":{\"value\":5.972968285148457}}},\"required_navi_type\":{\"type\":\"enum\",\"value\":2},\"task_name\":{\"type\":\"std::string\",\"value\":\"Navi\"}}],\"task_name\":{\"type\":\"std::string\",\"value\":\"Loop\"},\"times\":{\"type\":\"unsigned int\",\"value\":1}}]","script_name":"bzrobot_patrol.script"}]}
   */

  String? map_data;
  MapInfoBeanX? map_info;
  ObstaclesBeanX? obstacles;
  PositionsBeanX? positions;
  ProjectInfoBean? project_info;
  ScriptsBeanX? scripts;
}

@JsonSerializable()
class MapInfoBeanX {

	MapInfoBeanX();

	factory MapInfoBeanX.fromJson(Map<String, dynamic> json) => $MapInfoBeanXFromJson(json);

	Map<String, dynamic> toJson() => $MapInfoBeanXToJson(this);

  /**
   * map_info : {"height":457,"origin_theta":0,"origin_x":-5.50055380391934,"origin_y":11.983126061027363,"resolution":0.05,"width":582}
   * pgm_name : new_map.pgm
   * png_name : new_map.png
   */

  MapInfoBean? map_info;
  String? pgm_name;
  String? png_name;
}

@JsonSerializable()
class MapInfoBean {

	MapInfoBean();

	factory MapInfoBean.fromJson(Map<String, dynamic> json) => $MapInfoBeanFromJson(json);

	Map<String, dynamic> toJson() => $MapInfoBeanToJson(this);

  /**
   * height : 457
   * origin_theta : 0
   * origin_x : -5.50055380391934
   * origin_y : 11.983126061027363
   * resolution : 0.05
   * width : 582
   */

  int? height;
  int? origin_theta;
  double? origin_x;
  double? origin_y;
  double? resolution;
  int? width;
}

@JsonSerializable()
class ObstaclesBeanX {

	ObstaclesBeanX();

	factory ObstaclesBeanX.fromJson(Map<String, dynamic> json) => $ObstaclesBeanXFromJson(json);

	Map<String, dynamic> toJson() => $ObstaclesBeanXToJson(this);

  /**
   * obstacles : {"circles":[],"polygons":[],"polylines":[],"rectangles":[]}
   */
  ObstaclesBean? obstacles;
}

@JsonSerializable()
class ObstaclesBean {

	ObstaclesBean();

	factory ObstaclesBean.fromJson(Map<String, dynamic> json) => $ObstaclesBeanFromJson(json);

	Map<String, dynamic> toJson() => $ObstaclesBeanToJson(this);

  List<int>? circles;
  List<int>? polygons;
  List<int>? polylines;
  List<int>? rectangles;
}

@JsonSerializable()
class PositionsBeanX {

	PositionsBeanX();

	factory PositionsBeanX.fromJson(Map<String, dynamic> json) => $PositionsBeanXFromJson(json);

	Map<String, dynamic> toJson() => $PositionsBeanXToJson(this);

  List<PositionsBean>? positions;
}

@JsonSerializable()
class PositionsBean {

	PositionsBean();

	factory PositionsBean.fromJson(Map<String, dynamic> json) => $PositionsBeanFromJson(json);

	Map<String, dynamic> toJson() => $PositionsBeanToJson(this);

  /**
   * name : 1
   * position : {"theta":-1.8065404422243354,"world_x":19.569447416783788,"world_y":-4.297116858894512}
   * type : 2
   */

  String? name;
  PositionBean? position;
  int? type;
}

@JsonSerializable()
class PositionBean {

	PositionBean();

	factory PositionBean.fromJson(Map<String, dynamic> json) => $PositionBeanFromJson(json);

	Map<String, dynamic> toJson() => $PositionBeanToJson(this);

  /**
   * theta : -1.8065404422243354
   * world_x : 19.569447416783788
   * world_y : -4.297116858894512
   */

  double? theta;
  double? world_x;
  double? world_y;
  double? show_x;
  double? show_y;
  double? rect_left;
  double? rect_top;
  double? rect_width;
  double? rect_height;
  bool? isSelect;
}

@JsonSerializable()
class ProjectInfoBean {

	ProjectInfoBean();

	factory ProjectInfoBean.fromJson(Map<String, dynamic> json) => $ProjectInfoBeanFromJson(json);

	Map<String, dynamic> toJson() => $ProjectInfoBeanToJson(this);

  /**
   * map_name : map.json
   * obstacle_name : obstacle.json
   * positions_name : positions.json
   * project_id : cfb81e49-bafa-42f0-ab86-69beb515cdbd
   * project_name : 0711
   * project_stamp : 2020-07-11T01:50:22.287000Z
   */

  String? map_name;
  String? obstacle_name;
  String? positions_name;
  String? project_id;
  String? project_name;
  String? project_stamp;
}

@JsonSerializable()
class ScriptsBeanX {

	ScriptsBeanX();

	factory ScriptsBeanX.fromJson(Map<String, dynamic> json) => $ScriptsBeanXFromJson(json);

	Map<String, dynamic> toJson() => $ScriptsBeanXToJson(this);

  List<ScriptsBean>? scripts;
}

@JsonSerializable()
class ScriptsBean {

	ScriptsBean();

	factory ScriptsBean.fromJson(Map<String, dynamic> json) => $ScriptsBeanFromJson(json);

	Map<String, dynamic> toJson() => $ScriptsBeanToJson(this);

  /**
   * data : [{"break_point_flag":{"type":"bool","value":false},"id":{"type":"std::string","value":"EGWXFX1HJl"},"sub_tasks":[{"break_point_flag":{"type":"bool","value":false},"id":{"type":"std::string","value":"M9b62f9Y2P"},"input_waypoint":{"type":"waypoint","value":{"name":{"type":"std::string","value":"4"},"theta":{"type":"double","value":1.5707963267948966},"x":{"type":"double","value":4.606330198766207},"y":{"value":1.8762420583418162}}},"required_navi_type":{"type":"enum","value":2},"task_name":{"type":"std::string","value":"Navi"}},{"break_point_flag":{"type":"bool","value":false},"id":{"type":"std::string","value":"kT4uHe03IG"},"input_waypoint":{"type":"waypoint","value":{"name":{"type":"std::string","value":"5"},"theta":{"type":"double","value":1.5707963267948966},"x":{"type":"double","value":0.5096039719595664},"y":{"value":5.972968285148457}}},"required_navi_type":{"type":"enum","value":2},"task_name":{"type":"std::string","value":"Navi"}}],"task_name":{"type":"std::string","value":"Loop"},"times":{"type":"unsigned int","value":1}}]
   * script_name : bzrobot_patrol.script
   */

  String? data;
  String? script_name;
}
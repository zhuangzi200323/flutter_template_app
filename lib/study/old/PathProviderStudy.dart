...
class PathProviderStudy extends StatefulWidget {
  PathProviderStudy({Key? key, required this.title}) : super(key: key);
  final String title;
...
class _MyHomePageState extends State<PathProviderStudy> {
  late Future<Directory> _tempDirectory; //临时目录
  late Future<Directory> _appSupportDirectory;
  late Future<Directory> _appLibraryDirectory;
  late Future<Directory> _appDocumentsDirectory; //文档目录
  late Future<Directory?> _externalDocumentsDirectory;//外部文档目录
  late Future<List<Directory>?> _externalStorageDirectories; //外部存储目录
  late Future<List<Directory>?> _externalCacheDirectories; //外部缓存目录
